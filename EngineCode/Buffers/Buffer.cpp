...
{
	m_BufferSize = static_cast<engIntU32>(Params.m_Size);
	...
	VkResult result = vkCreateBuffer(Params.m_LogicalDevice, &bufferInfo, nullptr, &m_Buffer);
	...
	result = vkAllocateMemory(Params.m_LogicalDevice, &allocInfo, nullptr, &m_BufferMemory);
...
{
	void* mappedMemory;
	vkMapMemory(Device, m_BufferMemory, 0, m_BufferSize, 0, &mappedMemory);
	memcpy(mappedMemory, data, m_BufferSize);
...
{
	vkCmdDraw(CommandBuffer, m_BufferSize, 1, 0, 0);
}
